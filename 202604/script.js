...
async function init() {
    try {
        ...
        // 載入使用者手動新增的紀錄
        const manualRecords = JSON.parse(localStorage.getItem('manualRecords_202604') || '[]');
        ...
        // 載入使用者存在瀏覽器裡的手動修改紀錄
        const userEdits = JSON.parse(localStorage.getItem('userEdits_202604') || '{}');
        const orderEdits = JSON.parse(localStorage.getItem('orderEdits_202604') || '{}');
        
        // 載入被刪除的紀錄
        const deletedRecords = JSON.parse(localStorage.getItem('deletedRecords_202604') || '[]');
    ...
    document.querySelectorAll('.delete-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (confirm(`確定要刪除「${record.details}」這筆紀錄嗎？\n(刪除後若按下匯出儲存，此項目將會永久移除)`)) {
                ...
                // 從手動新增清單移除 (如果是手動新增的話)
                let manualRecords = JSON.parse(localStorage.getItem('manualRecords_202604') || '[]');
                manualRecords = manualRecords.filter(r => r.id !== id);
                localStorage.setItem('manualRecords_202604', JSON.stringify(manualRecords));
                
                // 標記為已刪除 (防止重整後從 data.json 又載入)
                let deletedRecords = JSON.parse(localStorage.getItem('deletedRecords_202604') || '[]');
                if (!deletedRecords.includes(id)) {
                    deletedRecords.push(id);
                    localStorage.setItem('deletedRecords_202604', JSON.stringify(deletedRecords));
                }
...
printBtn.addEventListener('click', () => {
    ...
    if (subtitleEl) {
        ...
        if (yearMatch && monthMatch) {
            ...
            yyyymm = `${yyyy}${mm}`; // 組合為 202604
        }
    }
    
    // 如果從副標題抓不到，試著從網址路徑抓取 (例如 /202604/)
    if (!yyyymm) {
...
addManualBtn.addEventListener('click', () => {
    ...
    // 初始化數值
    document.getElementById('manual-date').value = "2026-04-";
...
function saveLocalEdits() {
    ...
    localStorage.setItem('userEdits_202604', JSON.stringify(edits));
    localStorage.setItem('orderEdits_202604', JSON.stringify(orderEdits));
}
